...
const intentosExamen = new Map();

// Enviar examen con preguntas aleatorias al frontend ---  
const startQuiz = (req, res) => {
    const idUsuario = req.user.id;
    console.log(`Acceso al /api/questions/start - ${idUsuario}`);
    // Obtenemos el tipo de cuestionario desde el query
    const idQuiz = parseInt(req.query.idQuiz);
    console.log(`Iniciando cuestionario con ID: ${idQuiz}`);

    // Verificar si el usuario tiene un intento activo o completado para el cuestionario
    ...
    while (preguntasAleatorias.length < 8 && todoElQuiz.length > 0) {
        ...
        preguntasAleatorias.push({
            ...
            indiceCorrecto: indiceCorrecto //Guardamos
        });
    }
...
const submitAnswers = (req, res) => {
    ...
    const idUsuario = req.user.id;
    console.log(`Acceso al /api/questions/submit - ${req.user.name}`);
    ...
    const pasado = porcentaje >= 80; // necesita el 80% para aprobar
...
module.exports = { startQuiz, submitAnswers, estadoIntento };